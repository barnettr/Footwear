...
var fetchMenSelected = function () {
    ...
    var $fetchFirstPanel = function () {
        ...
        if ($v.length) {
            ...
            $activePanel.load(url, function (responseText, statusText, xhr) {    //  completeCallback has the following signature: function( status, statusText, responses, headers ) {}
                if (statusText == "error") {
                    $activePanel.text("<b>Error loading slide content: " + xhr.status + " " + xhr.statusText + "</b>");
    ...
    var errorCallback = function (data, title, thisSelectedIndex) {
        var $activePanel = jQuery(".tabs #tab-0" + thisSelectedIndex + "");
        $activePanel.load(data, function () {
            jQuery('span#title').html(title);
        });
    }
